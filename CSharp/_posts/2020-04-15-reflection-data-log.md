...
#### 평가
이제 어떠한 새로운 LoadData가 나오더라도 똑같은 방법으로 원하는 프로퍼티에 대해 로깅할 수 있다.
이처럼 Reflection을 활용하면 위와 같은 생각지도 못한 부분들조차 자동화시킬 수 있다.
Create함수의 파일로부터 데이터를 읽어오는 부분도 비슷한 방법으로 일반화시킬 수 있고,
실제로 테스팅, 전처리, 코드를 통한 코드 생성까지도 reflection을 이용하여 자동화할 수 있다.
